 ...
 */
const calculateOdds = (betOption: 'small' | 'large' | number): number => {
 ...
 */
const determineWin = (betOption: 'small' | 'large' | number, diceResult: number): boolean => {
  if (typeof betOption === 'string') {
    if (betOption === 'small') {
 ...
 */
export const rollDice = async (req: GameRequest, res: Response) => {
  try {
    ...
    // Calculate odds
    const odds = calculateOdds(betOption as any);

    // Roll the die
    const diceResult = rollDie();

    // Determine win/loss
    const win = determineWin(betOption as any, diceResult);
 ...
 */
export const getGameHistory = async (req: GameRequest, res: Response) => {
  try {
 ...
 */
export const getGameStats = async (req: GameRequest, res: Response) => {
  try {
